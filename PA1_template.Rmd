...
```{r intervalPlot, scatterplot}
par(mar=c(5,4,1,1),las=1)
plot(intervel,interval_average_steps,xlab="5-minute interval",ylab="Average number of steps",type="l")
```
...
```{r recomputerTotalSteps, echo=TRUE}
...
```{r weedaysCal, echo=TRUE}
impune_data$days<-"Weekdays"

impute_new<-unique(activity$date)

for(i in 1:length(impute_new)){
    if(weekdays(as.Date(impute_new[i]))=="Saturday" || weekdays(as.Date(impute_new[i]))=="Sunday"){
